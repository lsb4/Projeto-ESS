...
const Carousel = (props) => {
    ...
    const updateIndex = (newIndex) => {
        ...
        } else if (newIndex >= React.Children.count(children) - (numberOfElements - 1)) {
            newIndex = 0
        }
    ...
    return (
        <div className='control'>
            <div className='carousel'>
                <div className='inner' style={ { transform: `translateX(-${index*displaySize}%)` }}>
                    {React.Children.map(children, (el, i) => {
                        return React.cloneElement(el, {width: `${displaySize}%`})
                    })}
                </div>
            </div>
